...
def parse_items_json(raw) -> pd.DataFrame:
    ...
    try:
        ...
        df["数量"] = clean_number_series(df["数量"]).fillna(1).astype(int).astype(str)
        df["售价"] = clean_number_series(df["售价"]).fillna(0).astype(int).astype(str)
        df["折扣"] = clean_number_series(df["折扣"]).fillna(100.0).map(lambda x: str(int(x)) if float(x).is_integer() else str(float(x)))
        df["成本"] = clean_number_series(df["成本"]).fillna(0).astype(int).astype(str)
        return df
    except:
        return make_empty_items_df()
...
with st.sidebar:
    st.header("⚙️ 订单配置")
    client_name = st.text_input("客户姓名", key="client_input")
    quote_id = st.text_input("单号", key="quote_id_input")
    exchange_rate = st.number_input("参考汇率 (JPY->RMB)", key="rate_input", format="%.4f", step=0.0001)
    
    col_a, col_b = st.columns(2)
    service_fee_pct = col_a.number_input("服务费 %", key="service_pct_input")
    pay_fee_pct = col_b.number_input("通道费 %", key="pay_fee_pct_input")
...
# --- 主区域：商品录入 ---
st.markdown("### 📝 商品录入")
st.info("💡 提示：数字列已改为稳定文本输入，可直接输入 12000、95%、¥8000 等格式。")
...
edited_df = st.data_editor(
    ...
    column_config={
        ...
        "数量": st.column_config.TextColumn("数量", width="small", help="建议输入整数，例如 1、2、3"),
        "售价": st.column_config.TextColumn("售价(JPY)", width="small", help="可输入 12000、12,000、¥12000"),
        "折扣": st.column_config.TextColumn("折扣(%)", width="small", help="可输入 100、95、90.5、95%"),
        "成本": st.column_config.TextColumn("成本(JPY)", width="small", help="可输入 8000、8,000、¥8000"),
    }
)
...
df["商品"] = df["商品"].fillna("").astype(str).str.strip()
df = df[df["商品"] != ""].copy()

if df.empty:
    df = pd.DataFrame(columns=["商品", "数量", "售价", "折扣", "成本", "项原价", "项折后"])
else:
    df["数量"] = clean_number_series(df["数量"]).fillna(0).clip(lower=0)
    df["售价"] = clean_number_series(df["售价"]).fillna(0).clip(lower=0)
    df["折扣"] = clean_number_series(df["折扣"]).fillna(100.0).clip(lower=0, upper=100)
    df["成本"] = clean_number_series(df["成本"]).fillna(0).clip(lower=0)
